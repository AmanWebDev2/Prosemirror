...
function EmojiUploader(props) {
  ...
  const [expandedSelRangeofTextarea, setExpandedSelRangeofTextarea] =
    useState("");

  const showEmojiPopover = () =>
    setShowPopoverOf(showPopoverOf === "showEmoji" ? "" : "showEmoji");
  const getEmojiPosition = () => {
    let sel, range, expandedSelRange;
    sel = window.getSelection();
    range = sel.getRangeAt(0);
    if (
      sel.anchorNode?.parentElement.id === "contentEAditableInput" ||
      sel.anchorNode?.parentElement?.parentElement.id === "contentEAditableInput"
    ) {
    }
    range = sel.getRangeAt(0);
      ...
      >
        <svg focusable="false" aria-hidden="true" viewBox="0 0 18 18">
          <path
            ...
            fill={showPopoverOf === "showEmoji" ? "#6600ff" : "#ADADAD"}
          ></path>
        </svg>
      </button>
    </>
